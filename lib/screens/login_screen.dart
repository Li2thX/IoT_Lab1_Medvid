...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _autoLogin() async {
    ...
    if (email != null && password != null && _isConnected) {
      ...
      final isDataFresh = duration < Duration(hours: 1).inMilliseconds;
  ...
  void _loginUser() async {
    if (_validateLogin()) {
      // Витягуємо збережені дані з SharedPreferences
      final prefs = await SharedPreferences.getInstance();
      ...
      if (storedEmail != null && storedPassword != null) {
        // Перевірка введених даних з тими, що збережені
        if (inputEmail == storedEmail && inputPassword == storedPassword) {
          // Збереження часу входу та перенаправлення на головний екран
        ...
        _showErrorDialog('Користувач не знайдений, будь ласка, зареєструйтесь.');
      }
    }
  }
